...
local function blightNotification(event)
	...
	if (event.eventType == "weatherTransitionStarted") then
		...
		if not wasBlight and nextBlight then -- ѕогода мен€етс€ на бурю
			onBlightStart()
			return
		end
	end
	...
	if (event.eventType == "weatherTransitionFinished") then
		if not wasBlight and isBlight then -- ѕогода сменилась на бурю
			wasBlight = true
			return
		end
	end
	...
	if (event.eventType == "cellChanged") then
		if not wasBlight and isBlight then -- ¬ новой €чейке по€вилась бур€
			-- если загрузились в интерьере, а снаружи бур€ - уведомление не нужно
